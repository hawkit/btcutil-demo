...
func TestAppDataDir(t *testing.T) {
	...
	// Get the home directory to use for testing expected reulsts
	var homeDir string
	...
	tests := []struct {
		...
		// Single dot provided for application name, so expect current directory
		{"windows", ".", false, "."},
		{"windows", ".", true, "."},
		{"linux", ".", false, "."},
		{"darwin", ".", false, "."},
	...
	for i, test := range tests {
		...
		if ret != test.want {
			t.Errorf("appDataDir #%d (%s) does not match - " + "expected got %s, want %s", i, test.goos, ret, test.want)
		}
	}
